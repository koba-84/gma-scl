"""Preprocess WoS metadata into AAPD-compatible CSV splits."""
...
FILE_DIR = BASE_DIR / 'Meta-data' / 'Data.xlsx'
total_len = []
np.random.seed(7)
XLSX_NS = {'xlsx': 'http://schemas.openxmlformats.org/spreadsheetml/2006/main'}
...
def _xlsx_shared_strings(xlsx_file):
    """Read the shared string table from the WoS workbook."""
    root = ET.fromstring(xlsx_file.read('xl/sharedStrings.xml'))  # nosec B314
    shared_strings = []
    for item in root.findall('xlsx:si', XLSX_NS):
        text = ''.join(
            text_node.text or ''
            for text_node in item.findall('.//xlsx:t', XLSX_NS)
        )
...
def _cell_column(cell_ref):
    """Return the Excel column letters from a cell reference."""
...
def _cell_value(cell, shared_strings):
    """Resolve an xlsx cell value, including shared strings."""
    value = cell.find('xlsx:v', XLSX_NS)
...
def read_xlsx_rows(path):
    """Yield seven-column rows from the WoS xlsx workbook."""
    with ZipFile(path) as xlsx_file:
        shared_strings = _xlsx_shared_strings(xlsx_file)
        sheet = ET.fromstring(xlsx_file.read('xl/worksheets/sheet1.xml'))  # nosec B314
        for row in sheet.findall('.//xlsx:sheetData/xlsx:row', XLSX_NS):
            values = [''] * 7
            for cell in row.findall('xlsx:c', XLSX_NS):
...
def get_data_from_meta():
    """Read WoS metadata rows using the existing label selection rules."""
    data = []
    label_check = {}
    for line in list(read_xlsx_rows(FILE_DIR))[1:]:
        assert len(line) == 7
...
def get_label_columns(data):
    """Derive deterministic CSV label columns from first label appearance."""
    labels = []
    for line in data:
...
def write_csv(path, data, label_columns):
    """Write one split in the AAPD CSV file contract."""
    header = ['abstract']
    header.extend(str(i) for i in range(1, len(label_columns) + 1))
    with open(path, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f, quoting=csv.QUOTE_MINIMAL)
        writer.writerow(header)
        for line in data:
            active_labels = set(line['doc_label'])
            labels = [
                f"{float(label in active_labels):.1f}"
                for label in label_columns
            ]
            writer.writerow([line['doc_token'], *labels])


def write_label_descriptions(path, label_columns):
    """Write the generated numeric label mapping for WoS columns."""
    payload = {
        'dataset': 'WoS',
...
def split_train_dev_test(data, label_columns):
    """Split rows with the existing randomization and write CSV splits."""
    ...
    val = list(val)
    test = list(test)
    write_csv(BASE_DIR / 'train.csv', train, label_columns)
    write_csv(BASE_DIR / 'test.csv', test, label_columns)
    write_csv(BASE_DIR / 'dev.csv', val, label_columns)
...
def get_hierarchy(data):
    """Write the WoS label hierarchy sidecar file."""
    label_hierarchy = {}
    ...
    with open(BASE_DIR / 'wos.taxnomy', 'w') as f:
...
if __name__ == '__main__':
    dataset = get_data_from_meta()
    labels = get_label_columns(dataset)
    write_label_descriptions(BASE_DIR / 'label_descriptions.json', labels)
    get_hierarchy(dataset)
    split_train_dev_test(dataset, labels)
