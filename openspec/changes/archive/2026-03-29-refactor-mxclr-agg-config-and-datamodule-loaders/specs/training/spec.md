...
MXCLR configuration MUST keep common scalar hyperparameters in `configs/contrastive/model/mxclr.yaml`. Agg-family-specific graph builder settings MUST live under `configs/contrastive/model/agg/` and MUST be selected through Hydra defaults rather than a Python-side registry. Transport-based agg configs MUST default `transport_optimization` to `sinkhorn`.
...
- **AND** `contrastive.model.loss_fn.transport_optimization` は `sinkhorn` として解決される
